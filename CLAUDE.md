...
- **Asymmetric convergence (DJ-136).** The activity playbook is **one-iteration-shaped**; the outer loop is the runtime's job. On Claude Code that loop is the `/goal` evaluator (runs after every turn). On Codex / Gemini the loop is Locutus's `OuterLoopRunner` in `internal/runner/loop.go` (runs after every ACP session closes). Both read the playbook's plain-text verdict line (`converged: true` or `converged: false; <reason>`) to decide whether to re-dispatch, bounded by 20 iterations.
