...
- **`adopt` closes the spec → code gap (DJ-149).** Reads spec + state, identifies approaches needing work (missing for feat/strat in scope / unbound / spec-drifted / code-drifted / orphan-parent), dispatches the runtime to implement in stacked worktrees (`adopt/<NNN>-<approach-id>` with phase-N+1-branches-off-N), records state-asserted live status per DJ-068. Carries forward [DJ-087](docs/decisions/dj-087-approaches-are-synthesized-adopt.md)'s synthesize-on-adopt path: when a Feature or Strategy in scope has no Approach attached, adopt dispatches the `synthesizer` subagent (parent body + applicable decisions in), calls `spec_propose_approach` with id `app-<parent-id>` (deterministic, idempotent), and the new approach flows into the normal implement chain in the same iteration. refine builds the spec layer — decisions, features, strategies — but does NOT emit approaches; adopt owns synthesis. Partially supersedes DJ-148's state-side fields on `spec.Approach`; state re-established at `.borg/state/` per DJ-068/DJ-096. `ReconciliationState.SpecHash string` migrates to `SpecHashes map[string]string` keyed by spec id for granular one-hop upstream subgraph drift detection (catches DJ-138 cascade drift, refine revisions to approach itself, renames as coincident add+remove). Seven new MCP tools: write (`state_record_reconciliation`, `state_refresh_artifacts`, `state_mark_status`, `state_delete_record`) + read (`state_list_records`, `state_get_record`, `state_compare_hashes`); all captureOnly-wrapped per DJ-147. `state_compare_hashes` is read-only server-side spec-hash diff (added / removed / changed) per approach — agent calls it in adopt's Step 2 to detect spec drift without reproducing server-side hash bytes. Adds `drift-classifier` subagent (semantic-vs-trivial code drift judgment). Per-runtime convergence drivers from DJ-144. See [DJ-149](docs/decisions/dj-149-adopt-spec-to-code-reconciliation.md).
- **Deliverable-shape obligations as coverage-critic findings (DJ-150).** `spec-coverage-critic` runs at refine-time per identified deliverable shape, enumerates category obligations via grounded web search (no canned examples, no registry), and judges coverage by reading existing feature bodies in natural language. Uncovered obligations become findings; the architect's revise pass either extends an existing feature's body scope or proposes a new feature via `spec_propose_feature` to close the gap. No new graph node kinds, no `covers` citation slice on features, no persistent obligation registry — outcomes drive feature mutations via the existing `spec_propose_feature` / `spec_revise_feature` tools. Triggered by winplan's empty home-page surface after the first end-to-end adopt run (`next build` succeeded, 810 tests passed, `app/page.tsx` was a 9-line placeholder because no feature owned the public entry surface). Codifies the **topology-vs-process bias check** as a project-wide convention: before any future DJ proposes a new node kind, the proposed kind must correspond to a recognized PM/engineering artifact (ADRs, features, design docs, goal trees); otherwise the fix lives in prompts / critics / playbook prose. The critic dispatches under both refine (Phase 0 / Step 1 of each iteration, before the scout — per the DJ-150 amendment that moved it ahead of the scout so uncovered obligations feed the scout's convergence judgment on an already-converged graph) and assimilate (after gap-analyst reconciliation, before emit + approach synthesis) — one parallel dispatch per identified deliverable shape, with per-shape coverage judgment. Effectively idempotent across `refine goals` re-runs modulo grounded-enumeration stochasticity and deliverable-shape stability. **DJ-151 amends DJ-150**: the critic gains persona journey-walk enumeration as a second co-equal mode — walking each derived persona through the full deliverable lifecycle (arrival through departure) to surface the tacit/boilerplate obligations that citation-grounded web search structurally misses (nobody publishes an authoritative source for "apps need a sign-in page") — and reverses prose-coverage judgment to an ownership test: an obligation is covered only when a feature's declared scope claims the surface as its deliverable and an acceptance criterion exercises it, never ambient mention. See [DJ-150](docs/decisions/dj-150-spec-coverage-critic.md), [DJ-151](docs/decisions/dj-151-journey-walk-ownership-coverage.md), and `docs/agent-conventions.md § "Topology vs process"`.
