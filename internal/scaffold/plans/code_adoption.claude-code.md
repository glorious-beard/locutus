...
**Each implementation agent:** reads its plan file, writes and edits source files, runs the project's test suite, then calls `mcp__locutus__state_record_reconciliation` with the artifact map and `test_outcome: "passed"` or `"failed"`. If the project has no test suite, the agent scaffolds one with its native skills or halts the phase as operator-actionable rather than assuming `passed`. After the state tool returns, the agent `git add`s `.borg/state/<approach-id>.yaml` and includes it in the same phase commit as the source files — state records are version-controlled per DJ-068 / DJ-096 and belong with the code they describe. The same staging rule applies to any cross-phase `state_refresh_artifacts` or `state_mark_status` call (e.g., a trivial-drift fix on the tip branch): stage the updated YAML alongside whatever commit triggered the call.
