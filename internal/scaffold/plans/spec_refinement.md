...
- **MCP tools** (server `locutus`):
  - `spec_list_manifest` — compact index of every node, grouped by kind. No arguments. Start every iteration here.
  - `spec_get(ids: [...])` — batched body fetch. Pass every id you need in one call; never loop a per-id `spec_get`.
  - `spec_search(query, kind?, limit?)` — ranked free-text search. Use for topic-scoped questions ("what do we have on auth?").
  - `spec_propose_decision`, `spec_propose_feature`, `spec_propose_strategy` — upsert. Auto-commits per call; subscribers see `notifications/resources/updated` on `spec://manifest`.
  - `spec_revise_decision` — same shape as `spec_propose_decision`, but preserves `created_at` and rejects unknown ids.
...
- **Subagents** (published to your runtime under `locutus/`):
