...
4. **Dispatch `spec-goal-diff-matcher`.** Pass it the current `GOALS.md` content (from your earlier `Read` call) plus the array of existing nodes from step 3. The matcher returns a structured diff with four arrays: `unchanged`, `modified`, `deleted`, `added`. The diff is your work list for steps 5-7.
...
6. **Apply the diff.** For each entry the matcher returned:
   ...
   - `deleted` — call the matching `mcp__locutus__spec_delete_goal` or `mcp__locutus__spec_delete_antigoal` with the `id` and `reason` the matcher supplied.
...
7. **Persist the new hash.** Call `mcp__locutus__spec_update_goals_md_hash` with the `sha256:<hex>` value from step 1 and the current RFC3339 timestamp. The next iteration's short-circuit reads this value.

Keep track of which `goal-*` / `agoal-*` ids changed (anything in `modified`, `deleted`, or `added`). Step N+1's citation walk uses this set to decide which existing nodes need re-judgement.
