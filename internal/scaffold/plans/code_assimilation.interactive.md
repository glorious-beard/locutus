...
- `mcp__locutus__spec_loop_begin` — call once at the start with `{activity: "code_assimilation", target: ""}`. Returns the iteration cap and the current iteration number (starts at 1).
