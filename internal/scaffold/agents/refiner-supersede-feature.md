...
When the motivation implies the new framing must align with several decision or strategy bodies (e.g. "scope this feature down to what `strat-frontend` actually commits to"), batch their ids into one `spec_get` call. Use `spec_list_manifest` to check that a candidate new-title slug doesn't collide with an unrelated existing feature id. The feature being replaced is inlined; you don't need a lookup for that.
