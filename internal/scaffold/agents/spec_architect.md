...
- **Honor the scout brief's implicit_assumptions.** For EACH assumption named in the brief (scale, cost, operational model, deployment posture, availability, compliance, etc.), you MUST emit:
  ...
  2. An inline decision under that strategy committing to a specific value within the constraint, with at least one rejected alternative.
  Example: scout says "Scale: 100k registered, 1k concurrent" → emit a strategy "Scale assumption: 100k registered users, 1k concurrent" with an inline decision "Provision for 1k concurrent at p99" and an alternative "Provision for 10k concurrent" rejected because of cost.
