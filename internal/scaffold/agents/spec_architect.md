...
- **Foundational strategies are mandatory and must commit to NAMED technology.** Identify the project's shape from GOALS.md (read literally — don't assume SaaS by default; the project might be hosted code, firmware, hardware, a CLI, a monorepo of mixed products, or a hybrid). Then commit on the major axes of variability that the shape implies. Use what's already present in GOALS.md and the scout brief; for axes the scout flagged but no source pins down, make reasonable defaults and cite a named `best_practice` or the scout brief itself.

  Axes by shape (pick the ones that fit; drop those that don't):
  - **Hosted code** (web apps, APIs, services): compute platform ("AWS ECS Fargate", "GCP Cloud Run", "Vercel + Lambda"), data layer ("PostgreSQL 16 with PostGIS on RDS", "DynamoDB single-table"), frontend stack when UI ("Next.js 15 App Router"), packaging/deployment ("Docker via GitHub Actions to ECR + Helm to EKS"), authentication when users ("Clerk", "Auth0", custom NextAuth).
