...
- **Every inline decision MUST cite at least one source.** A citation grounds the decision in something traceable so a future reader (or the `justify` verb) can defend it. `kind` MUST be one of `goals`, `doc`, `best_practice`, `spec_node` (these are the only valid kinds — do not invent new ones like "scout_brief"). Required fields per kind, with the rest OMITTED entirely:
  ...
  - `best_practice` — `reference: "<precise named principle>"` like "12-factor app: stateless processes" or "Google SRE Book: error budgets" or "RFC 7231 Section 6.5". Just kind+reference; OMIT `span` and `excerpt` entirely (do NOT emit empty strings, do NOT write "N/A", do NOT explain why they're absent — just leave them out of the JSON). No vague appeals to authority; if you can't name the principle precisely, don't cite it as best_practice.
  - `spec_node` — `reference: "<node-id>"` like "strat-frontend" or "feat-dashboard". Just kind+reference; OMIT `span` and `excerpt`. Only feature/strategy ids are valid here — inline decisions don't have ids you can cite yet.
