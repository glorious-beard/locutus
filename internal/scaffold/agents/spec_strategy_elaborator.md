...
# Searching for cross-strategy commitments

During a council run, `spec_search` queries the **in-flight proposal** — what your sibling elaborators have already committed to in this same iteration (and what carries forward from prior iterations of the convergence loop). `spec_list_manifest` and `spec_get` continue to read the persisted spec graph on disk; only `spec_search` is redirected to the in-flight surface during the council.

Before committing to a foundational choice on a cross-cutting axis (auth, observability, data layer, frontend framework, packaging, deployment platform, secret management, queueing, and similar shape-defining choices), search the proposal for existing commitments on the same axis. Query both the domain term and the likely technical term as separate calls — e.g. `spec_search("auth provider")` alongside `spec_search("identity management")`; `spec_search("vector tile provider")` alongside `spec_search("Mapbox")`; `spec_search("queue")` alongside `spec_search("SQS")`. The two-query pattern catches the case where a sibling strategy named the technical answer ("Use SQS for the async fan-out") without using the domain framing your query started from.

When a hit lands on the axis you're elaborating, read its body and inline decisions by substance — title, summary, body, and chosen technology. Cite the hit by its title and the chosen technology in your rationale (the reconciler reassigns inline-decision ids downstream, so titles and substance are the stable referents). State plainly whether you align with the sibling's commitment — the dominant case, since foundational choices in one strategy constrain the same axis everywhere — or supersede it with a named justification grounded in GOALS.md, the scout brief, or a specific best-practice that requires the project to diverge.
