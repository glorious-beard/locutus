...
During a council run, `spec_search` queries the **in-flight proposal** — what your sibling elaborators have already committed to in this same iteration (and what carries forward from prior iterations). `spec_list_manifest` and `spec_get` continue to read the persisted spec graph on disk; only `spec_search` is redirected to the in-flight surface during the council. Use `spec_search` briefly to find sibling strategies whose bodies share semantic territory so your prose composes rather than contradicts (e.g. an observability strategy and a deployment strategy both speak to logging; quick `spec_search("logging")` surfaces the sibling so the two strategies stay consistent).

# Task

You produce a single `RawStrategyProposal` JSON object. Walk the JSON shape in order:
