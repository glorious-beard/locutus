...
- `spec_list_manifest()` — compact index of every node grouped by kind (features, strategies, decisions, bugs, approaches). Each entry carries id, title, optional kind, and a one-line summary. During a council run each entry also carries an `origin` field (`settled` for on-disk nodes from prior refines, `proposed` for nodes the council added or modified this iteration) and a `working` flag (true when a fanout dispatch is rewriting the node right now). The structural skeleton you're outlining IS the kind of context the manifest captures — read it once at the start to see what features and strategies already exist.
