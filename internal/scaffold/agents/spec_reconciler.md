...
- **`dedupe`** — Two or more inline decisions that reach the same conclusion via similar reasoning. Pick a canonical body (synthesized from the cluster, preserving the strongest rationale and citations). All sources will be rewritten to reference the canonical decision.
- **`resolve_conflict`** — Two or more inline decisions that give incompatible answers to the same underlying question (e.g., one feature says "Use Postgres" and another says "Use ClickHouse" for the same workload). Pick a `canonical` decision with industry-best-practice and ecosystem popularity as the priority. Record the `loser`'s title + rationale + the reason it was rejected; the assembler appends them to the `canonical` decision's `alternatives[]`. (Field names match the schema verbatim — the surviving decision is `canonical`, not `winner`.)
- **`reuse_existing`** — A cluster of inline decisions matches an existing-spec decision provided in the snapshot. Reference the existing decision's ID rather than minting a new one.
