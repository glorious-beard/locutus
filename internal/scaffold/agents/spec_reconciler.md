...
- **`resolve_conflict`** — Two or more inline decisions that give incompatible answers to the same underlying question (e.g., one feature says "Use Postgres" and another says "Use ClickHouse" for the same workload). Pick a winner with industry-best-practice and ecosystem popularity as the priority. Record the loser's title + rationale + the reason it was rejected; the assembler appends them to the winner's `alternatives[]`.
...
When two decisions conflict, pick a winner using these criteria, in order:
