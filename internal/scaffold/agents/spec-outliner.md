...
The `spec_list_manifest`, `spec_get`, and `spec_search` tools let you inspect the spec graph. The structural skeleton you're outlining IS the kind of context the manifest captures — read it once at the start to see what features and strategies already exist. `spec_get` is rarely needed at the outline stage; manifest summaries are usually enough.

Use `spec_search` for "does this concept already exist?" checks during authoring — it's the fastest way to find an id you might want to reuse instead of minting a duplicate. Example: before adding a feature or strategy to the outline, run `spec_search("<topic>")` to see whether the concept exists under a different name — this is the cheapest place to catch a near-duplicate, because minting a new id at the outline stage commits every elaborator downstream to building against the duplicate.

Scan the manifest before outlining — reuse existing feature and strategy ids in your output rather than minting new slugs that duplicate concepts already in place. The elaborators downstream rewrite the BODY of nodes you reference by existing id; minting a new id for an existing concept would create the duplicate the architect-side reconcile can't dedupe (because there's nothing to compare against at the outline level).
