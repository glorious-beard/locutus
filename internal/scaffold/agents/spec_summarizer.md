...
3. **"What", not "why" — and not "what isn't".** The summary says what was decided / built / committed, in positive terms. Rejected alternatives, comparison phrases ("rejecting X", "instead of Y", "rather than Z"), and rationale clauses ("because…", "as its…", "to ensure…") all belong in `rationale`, `architect_rationale`, or the structured `alternatives` array on the node — never in `summary`. An agent scanning the manifest is asking "is this the node I want?" — telling them what the node ISN'T is noise.
...
- **Decision** — what was chosen, in positive committing terms. ("Adopt Postgres with logical replication for the primary OLTP store." — not "Adopt Postgres; reject MySQL.") The chosen option is the "what"; alternatives the decision considered live in the node's `alternatives` array and are queried separately when needed.
