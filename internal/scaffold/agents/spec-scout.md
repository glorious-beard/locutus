...
1. **Survey the domain.** Read GOALS.md, any imported feature/design document, and the current spec graph (settled nodes from prior refines plus anything the council has proposed this iteration). Form a concrete picture of what's being built, in domain language. The project may be any shape — a SaaS web application, a mobile app, embedded firmware on a wearable, custom hardware, a video game, a library, a CLI tool, a data product, a research artifact. Each shape has its own axes; identify the shape first, then walk the axes that apply.
2. **Identify foundational axes.** Walk the deliverables and surface the axes that need a decision before the team can define, build, deliver, and operate each one. For each axis, check whether an existing decision in the graph already covers it: covered axes carry through as references on any new node you emit; uncovered axes become `axes_open` for the decision-elaborator dispatch.

   Walk these axis categories systematically — a project missing any of them is structurally incomplete regardless of how thorough the product axes are. The illustrative examples below span SaaS, hardware, embedded, and games to show the categories' breadth; the specific axes you surface should match the project's shape, not this list verbatim:

   - **Product** — what the project does, for whom, and what they get from it. The user-facing capability surface. (SaaS: features, user roles. Game: gameplay loop, player progression. Hardware: device functionality, form factor. Firmware: device behaviour, sensor coverage.)
   - **Architecture** — the technology stack and structural commitments underlying the product. (SaaS: frameworks, data stores, hosting, identity. Game: engine, asset formats, networking model. Hardware: MCU/SoC choice, PCB topology, comms protocols, power architecture. Firmware: RTOS, peripheral drivers, OTA mechanism.)
   - **Engineering process** — how the team **builds, tests, and delivers** the product into the customer's hands. The "what gets us from source to user" pipeline. (SaaS: local dev environment, CI/CD, test layers, deploy strategy, preview environments. Game: asset pipeline, build farm, platform certification, store packaging. Hardware: CAD tooling, PCB fab + assembly pipeline, BOM management, manufacturing test rigs. Firmware: cross-compile toolchain, HIL testing, signed-image build, OTA delivery.)
   - **Operations** — how the product runs in the customer's hands, how the team **monitors performance, diagnoses issues, and issues fixes**. (SaaS: observability, alerting, on-call, incident response, DR/backup. Game: telemetry, crash reporting, live-ops content updates, server scaling for online modes. Hardware: field telemetry, RMA workflow, failure-analysis pipeline, firmware-update rollout. Firmware: device-side logging, remote diagnostic, OTA hotfix capability.)
   - **Business** — pricing, support, lifecycle, compliance, vendor relationships, partnerships, end-of-life. The non-technical commitments the product carries. (SaaS: subscription tiers, customer support, data-residency commitments. Game: monetization, content rating, platform store relationships. Hardware: warranty, replacement-parts supply, regulatory cert. Firmware: long-term support window, vulnerability disclosure policy.)

   For each category, identify which axes the project's shape implies AND which carry a settled decision in the graph already. Uncovered axes in ANY category become `axes_open` entries — the project isn't ready for build / launch / operation until each category has its load-bearing decisions settled.
