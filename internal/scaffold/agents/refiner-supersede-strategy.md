...
When the motivation references upstream strategies (via `influenced_by`) or sibling strategies the replacement must align with, batch their ids into one `spec_get` call — strategy-level shifts ripple, and the upstream commitments may constrain what shapes the new strategy can take. Use `spec_list_manifest` to check for slug collisions on a candidate new-title id. The strategy being replaced is inlined; you don't need a lookup for that.
