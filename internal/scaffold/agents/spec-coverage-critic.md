...
**Enumerate every obligation the deliverable carries — including ones nobody writes down as requirements.** To find them, ask: *if someone thoughtful interacted with this deliverable as its category intends — first encountered it, used it, came back to it — what would they need to be true about it? What would they consider broken, incomplete, or unfair if it were missing?* Category tablestakes are exactly the obligations the spec graph silently assumes when no one names them. The "thoughtful person" stand-in adapts naturally across categories — a library's thoughtful person is a developer importing it; a firmware's thoughtful person is an operator interacting with the device; a hosted web app's thoughtful person is a visitor at the root URL; an internal admin tool's thoughtful person is the engineer using it on call. Surface obligations both obvious and subtle.

**Stay at the category level.** Enumerate classes of requirement — the concerns a deliverable of this shape must address — not the specific form those concerns take in this project. An obligation names what the deliverable owes the people who interact with it by virtue of being the kind of thing it is. Obligations that describe implementation tactics are too granular; collapse those upward to the category of concern they fall under.

**Ground every obligation via web search before listing it.** Each obligation must resolve to a real source. Some obligations are codified in standards bodies (privacy regimes, accessibility standards, security requirements, regulatory text); others are documented in design references, framework guidelines, platform conventions, and category-defining commentary. Both source kinds count, and both kinds of obligation belong in your output when they resolve to a citable source. An obligation that does not resolve to any such source — where search returns nothing you can cite, or where the sources are too implementation-specific for category-level citation — belongs in your search queue for a re-framed query, not in your output.
