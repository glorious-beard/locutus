...
type Request struct {
	...
	// FormatExampleProse is the labeled-prose rendering of the
	// OutputSchema's registered example payload, generated by
	// reflection-walking the example struct. Each top-level field
	// becomes a ## section carrying the field's jsonschema
	// description + the example value rendered as inline scalar or
	// nested labeled list. Populated by the executor alongside
	// FormatExampleDoc.
	//
	// Used in two places by the adapter's runSplit:
	//
	//   - Reasoning pass: emitted as a Cacheable user message BEFORE
	//     the projected input. Demonstrates the kind of prose
	//     narration the reasoner should produce — section-anchored
	//     content per field, no JSON syntax. Complements the prose
	//     directive that trails the projected input.
	//   - Format pass: emitted alongside FormatExampleDoc as a
	//     one-shot input → output pairing ("here's representative
	//     reasoning prose for this schema; here's the JSON it maps
	//     to"). Helps the formatter's edge cases without authoring
	//     new content.
	//
	// And by the single-call path (executor.runOne for thinking-off
	// agents with schema): emitted as a Cacheable user message
	// before the projected input, replacing the JSON dump that
	// previously lived in BuildSystemPrompt's appended ## Example
	// output block. Same content surface; cleaner cache layering
	// (system prompt stays byte-stable across calls) and avoids
	// priming JSON-mode output via the system prompt.
	//
	// Empty when no example is registered or the example isn't a
	// struct. The adapter then skips the prose layer.
	FormatExampleProse string
