...
type Workstream struct {
	...
	Steps          []PlanStep             `json:"steps,omitempty" yaml:"steps,omitempty" jsonschema:"description=DEPRECATED under DJ-121. Pre-planned step decomposition the coding agent may consult as a starting hint. The agent owns its own decomposition via _locutus/checklist.md; this field is retained for transitional planner output and is ignored by the supervisor when Workstream.Assertions is populated. Phase 9 of DJ-121 removes the field."`
	Assertions     []Assertion            `json:"assertions,omitempty" yaml:"assertions,omitempty" jsonschema:"description=Acceptance criteria that must hold when this workstream completes. Under DJ-121 this is the primary validation contract — the validator grades against these criteria at workstream completion. Replaces the per-step assertions on PlanStep.Assertions (deprecated)."`
}
...
type PlanStep struct {
	...
	Assertions    []Assertion       `json:"assertions,omitempty" yaml:"assertions,omitempty" jsonschema:"description=DEPRECATED under DJ-121. Validation checks scoped to this step. The supervisor's validator prefers Workstream.Assertions when populated; this field is honored only as a transitional fallback when the planner has not yet been migrated to emit workstream-level criteria. Phase 9 of DJ-121 removes the field along with PlanStep itself."`
