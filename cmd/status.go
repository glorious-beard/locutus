...
func GatherStatus(fsys specio.FS) render.StatusData {
	...
	// Collect ids of approaches invalidated by `refine --supersede`
	// and not yet regenerated by `adopt`. Sorted for deterministic
	// rendering. WalkPairs would re-attempt frontmatter parsing on
	// every approach, but the data we need lives in the Approach
	// frontmatter — use specio's per-file Markdown loader to keep
	// per-file errors from masking bulk counts.
	if files, err := fsys.ListDir(".borg/spec/approaches"); err == nil {
		...
		for _, f := range files {
			...
			a, _, err := specio.LoadMarkdown[spec.Approach](fsys, f)
			if err != nil {
				continue
			}
			if a.IsInvalidated() {
		...
		sd.InvalidatedApproaches = invalidated
	}
