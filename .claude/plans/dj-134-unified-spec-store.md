...
> **Governing DJ:** [DJ-134](../../docs/DECISION_JOURNAL.md#dj-134). The DJ is the authoritative design record; this plan tracks **progress against** the DJ and captures session-level implementation notes.
>
> **Status:** DONE — Phases 1-4 landed 2026-05-23 (commits `3f336dd`, `6accb0c`, `374b173`, `d0e1107`). The unified SpecStore is the foundation layer DJ-135's MCP server tool surface runs against; the per-entry typed origin tag (settled / proposed / working) is what the MCP `spec_get` / `spec_search` / `spec_list_manifest` tools expose. Empirically validated against winplan refine 2026-05-26 (`~/projects/winplan/.locutus/sessions/20260526/1918/080000/`) where the playbook-executed council mutated the store through MCP tool calls and the resulting graph rendered correctly.
...
- [internal/agent/spec_store.go](../../internal/agent/spec_store.go) — the unified store. Concretely:
  ...
  - Reads: `ListManifest() SpecManifest`, `GetSpec(ids []string) SpecGetResult`, `Search(query string, opts search.Options) ([]search.Hit, int, error)`.
  - **Batched `GetSpec` is the contract from day one**, not a follow-up refactor. The May 2026 winplan trace at `~/projects/winplan/.locutus/sessions/20260523/1347/33-a750b3/calls/0049-spec_scout/` exhausted the 10-round tool-use cap because the scout was issuing one `spec_get(id)` per concern across rounds; with 8 open concerns each touching ~1-2 nodes the per-call sequential pattern is structurally guaranteed to spiral. Forcing batching at the schema level (single tool, plural `ids` field, no scalar variant) removes the model's choice — there is no one-at-a-time API to fall back to. `SpecGetResult` is `{ Found map[string]json.RawMessage, Missing map[string]SpecGetMiss }`: partial success per id; per-id misses carry structured recovery info (Reason + either Suggestions for FS near-misses or AvailableIDs for the in-flight kind-matched set) rather than prose the model has to parse.
  ...
  - Internal: index update on every `Put`; not-found-recovery (inline kind-matched id list per missing id) carries over from `InFlightSpecStore.GetSpec` but folded into `SpecGetMiss.AvailableIDs` rather than prose.
...
- `TestSpecStore_LoadFromFSAllKinds` — every JSON file under `.borg/spec/{decisions,features,strategies,bugs,approaches}/` round-trips through `New` and is visible via `ListManifest` / `GetSpec([id])` with `Origin: settled`.
- `TestSpecStore_PutTagsAsProposed` — `store.Put(feature, OriginProposed)` makes the feature visible via `ListManifest` (with `Origin: proposed`) and `GetSpec([id])`; `Search` includes it in results.
- `TestSpecStore_GetSpecBatchedReturnsPartialResult` — `GetSpec(["dec-known", "dec-unknown"])` returns `{Found: {"dec-known": <body>}, Missing: {"dec-unknown": {Reason: ..., AvailableIDs: [...]}}}`. No top-level error; both ids represented exactly once.
- `TestSpecStore_GetSpecBatchedEmptyIdsIsEmptyResult` — `GetSpec(nil)` and `GetSpec([])` return `{Found: {}, Missing: nil}`, no error. Defensive: trims whitespace-only entries so accidental empty strings don't generate spurious misses.
...
- `TestSpecStore_GetSpecMissPopulatesAvailableIDs` — `GetSpec(["dec-no-such-axis"])` returns `Missing["dec-no-such-axis"]` with `AvailableIDs` containing every dec- id in the store and an empty `Suggestions` slice. Mirrors the existing InFlightSpecStore.GetSpec recovery contract but as structured data instead of a prose error.
- `TestSpecStore_GetSpecMalformedIDLandsInMissing` — `GetSpec(["bad id"])` returns `Missing["bad id"]` with `Reason` naming the regex violation; the top-level error is nil (per-id failures don't bubble up).
...
- [internal/agent/spec_tools.go](../../internal/agent/spec_tools.go) `RegisterSpecTools` — signature collapses to `(registry *ToolRegistry, store *SpecStore)`. Each of the three tool handlers (spec_search, spec_list_manifest, spec_get) calls into `store` directly. The SwappableSpec* indirections retire from the handler bodies. **spec_get's input schema becomes `{ids: [string]}`** (no scalar `id` variant), with the `ToolDef.Description` updated to describe the batched contract verbatim — *"Returns the full JSON of N spec nodes by id in one call. Input is an array of ids; output partitions into found (id → body) and missing (id → recovery info). Prefer one batched call over N sequential ones — sequential single-id calls across rounds cost rounds against the tool-loop cap."* `SpecGetInput.ID string` changes to `SpecGetInput.IDs []string`.
...
- [docs/agent-conventions.md](../../docs/agent-conventions.md) — add a new section under "Patterns to prefer" titled **"Tool descriptions live in registration, not prompts"**. The rule: when a prompt references a tool (spec_*, future write tools, anything in the registry), workflow guidance is fine in the prompt (*"call spec_get with every id you'll need from the open concerns + axes lists"*); tool-behavior descriptions (*"each manifest entry carries an `origin` field..."*) belong in the `ToolDef.Description` and `InputSchema.properties[].description` at `RegisterSpecTools`. The model reads tool descriptions on every call; duplicating them in the system prompt is bloat, and the two sources drift. Caught by walking new prompt edits with the question "is this WHEN-to-use or WHAT-the-tool-does?" The latter goes to registration.
- **Prompt sweep** — walk every agent file under [internal/scaffold/agents/](../../internal/scaffold/agents/) that references `spec_list_manifest`, `spec_get`, or `spec_search`. Today there are 19 such files; line-level audit count is ~101 references. For each: (a) strip tool-behavior text that duplicates the registered description (the `origin` / `working` field explanations, the kind-prefix-routing detail, the "during a council run these tools..." preamble all move from prompts to registration); (b) update singular `spec_get(id)` workflow phrases to plural `spec_get(ids)`; (c) where the agent legitimately needs to fetch multiple node bodies (scout grading concerns, critic-elaborator inspecting candidates, reconciler walking the in-flight proposal), add one explicit workflow sentence: *"enumerate every id you'll need upfront from the input lists, then issue one `spec_get` call — sequential single-id fetches across rounds cost rounds against the tool-loop cap."* The audit is mechanical but mandatory before the new tool surface ships; per `[[feedback-agent-conventions-checklist-first]]` walk `docs/agent-conventions.md` (including the new tool-description rule) as a checklist for each prompt touched.
...
1. Read DJ-134 in full ([docs/DECISION_JOURNAL.md#dj-134](../../docs/DECISION_JOURNAL.md#dj-134)). It's the authoritative design; this plan is progress tracking.
...
- **Re-architecting the model-facing tool surface beyond spec_get's batched signature.** `spec_search` and `spec_list_manifest` are shape-identical pre- and post-DJ-134. Only `spec_get` changes — scalar `id` → array `ids`, scalar body → partitioned `{found, missing}` result. The prompt sweep is in scope because today's prompts duplicate tool-behavior text that belongs at registration; that's a hygiene fix tied to the new tool description, not a re-architecture.
