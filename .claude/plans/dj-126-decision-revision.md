...
Phases 1-6 landed in a single sitting against a clean main branch with DJ-125 already merged. Notes:

- **Phase 1.** Revise-mode section added under `# Revise mode` heading after the citation-kind enum and before `# Mandates`. Walked [docs/agent-conventions.md](../../docs/agent-conventions.md) before drafting: described the revise inputs and behaviours positively (no "don't do X" lists), folded constraints into the task body rather than a trailing section, kept the existing `thinking: on` frontmatter because the agent emits a free-text rationale field where the model can show its work. Two scaffold tests guard the section + the axis-verbatim mandate ([TestDecisionElaboratorReviseModeSectionPresent](../../internal/scaffold/scaffold_test.go), [TestDecisionElaboratorPromptPreservesAxesInRevise](../../internal/scaffold/scaffold_test.go)).
- **Phase 2.** Added `hasReviseableConcerns`, `fanoutReviseableConcerns`, `decisionToRawProposal`, `projectReviseDecision`, and the `reviseableConcernItem` shape in [internal/agent/workflow_spec_generation_dj124.go](../../internal/agent/workflow_spec_generation_dj124.go). The loop template in [internal/agent/workflow_spec_generation.go](../../internal/agent/workflow_spec_generation.go) gained a `revise-decisions` step between `narrative` and `reconcile` (the `reconcile` step's `DependsOn` moved from `narrative` to `revise-decisions`; the executor marks conditional-skipped steps completed so the dependency holds when the revise step is skipped). Tests: [TestHasReviseableConcernsRequiresOpenAndRelatedIDs](../../internal/agent/workflow_spec_generation_dj124_test.go), [TestFanoutReviseableConcernsEmitsOnePerConcern](../../internal/agent/workflow_spec_generation_dj124_test.go), [TestProjectReviseDecisionIncludesPriorDecisionFullBody](../../internal/agent/workflow_spec_generation_dj124_test.go), [TestReviseDecisionsStepConditionalSkipsWhenNoOpenConcerns](../../internal/agent/workflow_spec_generation_dj124_test.go).
- **Phase 3.** `mergeDecisions` rewritten to detect axis intersection: zero matches appends (first-author path), exactly-one match replaces in place (preserving the prior id), multi-match records an integrity-violation concern and skips. The driving-concern → revised-decision link is recovered post-replace by walking `state.Concerns` for entries naming the replaced id rather than threading a concern id through fanout-item metadata — simpler and handles "one concern flags multiple decisions" naturally. Tests: [TestMergeDecisionsReplacesWhenAxesIntersectExactlyOne](../../internal/agent/workflow_spec_generation_dj124_test.go), [TestMergeDecisionsAppendsWhenNoAxisIntersection](../../internal/agent/workflow_spec_generation_dj124_test.go), [TestMergeDecisionsRecordsAmbiguityWhenMultipleExistingMatch](../../internal/agent/workflow_spec_generation_dj124_test.go), [TestMergeDecisionsMarksDrivingConcernAddressed](../../internal/agent/workflow_spec_generation_dj124_test.go).
- **Phase 4.** `PlanningState.AxisRevisionCount` field added (deep-copied in `snapshotPlanningState`). `incrementAxisRevisionCounts` runs on every replace match; `axesExceedingRevisionCap` is read by the scout spawner BEFORE the budget check so the cap fires with its own `convergence_revision_capped` DJ-103 event rather than draining to budget. `readDecisionRevisionCap` reads `LOCUTUS_DECISION_REVISION_CAP` with default 3. Tests: [TestAxisRevisionCountIncrementsOnReplace](../../internal/agent/workflow_spec_generation_dj124_test.go), [TestRevisionCapTerminatesWhenExceeded](../../internal/agent/workflow_spec_generation_dj124_test.go), [TestRevisionCapPerAxisIndependent](../../internal/agent/workflow_spec_generation_dj124_test.go), [TestDecisionRevisionCapEnvOverride](../../internal/agent/workflow_spec_generation_dj124_test.go).
- **Phase 5.** Implemented the side-channel pattern rather than threading a historian parameter through `mergeDecisions` (which is called by 9+ test sites): `PlanningState.PendingDecisionRevisedEvents` captures one entry per replace match inside `mergeDecisions`; `mergeDecisionsRecording(historian)` wraps the merge in the loop template and drains the slice through the historian after each call. Driving concerns are snapshotted BEFORE `markConcernsAddressedByRevision` flips statuses so the event's rationale carries the as-flagged finding text. Tests: [TestDecisionRevisedEventRecordedOnReplace](../../internal/agent/workflow_spec_generation_dj124_test.go), [TestDecisionRevisedEventIncludesDrivingConcern](../../internal/agent/workflow_spec_generation_dj124_test.go), [TestMergeDecisionsRecordingNilHistorianIsNoOp](../../internal/agent/workflow_spec_generation_dj124_test.go).
- **Phase 6.** [TestLoopConvergesAfterForcedContradictionViaRevision](../../internal/agent/workflow_spec_generation_dj124_test.go) drives the happy path end-to-end with `MockExecutor`: iter-1 critique flags `dec-datadog-obs` on cost; iter-2 revise-decisions dispatches and the mock revises to `dec-cloudwatch` on the same axis; the merge replaces in place and marks the cost concern as addressed; iter-2 scout returns Converged=true. Assertions cover the preserved id, the revised body in `ProposedSpec`, and a single `decision_revised` DJ-103 event with the cost concern's text in the rationale.
- **Test suite.** `go test ./... -count=1 -race -skip TestCLISinkRendersAgentLifecycle` clean; `go vet ./...` clean; `go build ./...` clean.

Phase 7 (live winplan re-run) requires running the binary against the winplan project — a manual session not appropriate for this implementation pass. Phase 8 (DJ-126 status `proposed` → `shipping`) remains gated on Phase 7 per the plan's pre-conditions; the implementation is in main but the architecture proof against winplan is the open item.
