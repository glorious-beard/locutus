...
>
> **Status:** DONE — implemented 2026-05-27 across 8 phases in `main`; Phase 9 (docs + DJ status flip) landed 2026-05-27. Phase 1 — `spec.Goal` / `spec.AntiGoal` + `SpecStore` extension and on-disk persistence under `.borg/spec/goals/` and `.borg/spec/antigoals/`; Phase 2 — optional `Advances` / `Respects` citation fields on existing kinds; Phase 3 — six new MCP write tools (`spec_propose_goal` / `spec_revise_goal` / `spec_delete_goal` + AntiGoal variants) plus `goal_deleted` / `antigoal_deleted` history events; Phase 4 — `goals_md_hash` + `goals_md_synced_at` on `.borg/manifest.json` with the atomic update path via `spec_update_goals_md_hash`; Phase 5 — `spec-goal-diff-matcher` canonical agent prompt; Phase 6 — `refine goals` reorganized sync → iterate → cite with hash short-circuit + bootstrap affordance + at-risk surface; Phase 7 — `feature_ingestion` reads goal layer + drafts `GOALS.md` diffs for plausibly-extensible carve-outs; Phase 8 — `locutus status --full` surfaces goal-layer counts + features-without-anchors; Phase 9 — CLAUDE.md / docs/mcp.md / docs/activities.md / docs/agent-conventions.md / docs/debugging-traces.md updated, DJ-139 + plan status flipped. Empirical validation against winplan deferred to the operator's first real `locutus refine goals` invocation from their own environment.
>
...
- Test: [internal/agent/spec_store_manifest_test.go](../../internal/agent/spec_store_manifest_test.go) — covers atomic read-modify-write semantics via the `SpecStore.GoalsMdHash` / `UpdateGoalsMdHash` methods; concurrent writes serialize through the store's mutex (post-review-fix restructure: free-function helpers were replaced with methods to fix the lost-update race and remove the over-broad `FS()` escape hatch).
