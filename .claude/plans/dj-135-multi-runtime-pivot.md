...
> **Governing DJ:** [DJ-135](../../docs/DECISION_JOURNAL.md#dj-135). The DJ is the authoritative design record; this plan tracks **progress against** the DJ and captures session-level implementation notes.
>
> **Status:** DONE — Phases 1-7 landed 2026-05-25 / 2026-05-26 across 35 commits on branch `dj-135-phase-1-mcp-foundation` (merged to main 2026-05-26). Phase 5 ckpt 4 first validated the end-to-end path against a fresh `/tmp/lt-p5c4-empirical*` project with Claude Code via `claude-agent-acp`: ACP attach, MCP server attach, playbook delivery, scout dispatch, candidate-survey fanout, decision-elaborator dispatch, `mcp__locutus__spec_propose_decision` invocation. Convergence-by-construction discipline locked via schema loosening (`axes` backfills from id) and playbook tightening (explicit first-action directive + prefixed MCP tool names). Council deletion in ckpt 5 removed ~51k lines across 207 files; net branch size +5k / −60k.
>
> Subsequent empirical validation across two successful winplan refine runs confirmed the architecture under real-project conditions: the 2026-05-25 first winplan refine produced the 27 / 9 / 6 baseline; the 2026-05-26 follow-up at `~/projects/winplan/.locutus/sessions/20260526/1918/080000/` identified the engineering-process / operations / business axes missed by round 1, converged in 3 iterations, and grew the graph to 42 / 12 / 10 — demonstrating that sequential refines productively compound. Declared baseline 2026-05-26; all prior DJs whose machinery this pivot subsumed are status-flipped to SUPERSEDED in the same sweep.
>
> Codex and Gemini empirical validation deferred — Claude Code only for v1. Per-runtime publisher format adjustments land alongside each validation, not pre-emptively. Follow-on refinements (per-runtime idiomatic dispatch, native goal-loops, hook configs) tracked under DJ-136. See `git log main..dj-135-phase-1-mcp-foundation` for the original branch commit list before the merge.
>
...
1. **Read DJ-135 in full** ([docs/DECISION_JOURNAL.md#dj-135](../../docs/DECISION_JOURNAL.md#dj-135)). The DJ is the authoritative design; this plan is progress tracking.
