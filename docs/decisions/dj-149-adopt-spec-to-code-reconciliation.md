...
4. **Approach synthesis for unbound feat/strat (per [DJ-087](dj-087-approaches-are-synthesized-adopt.md)).** Before drift classification, adopt walks `spec_list_manifest`'s `Features` and `Strategies` to find any node in scope whose `approaches[]` is empty (or whose cited `app-<parent-id>` doesn't exist in the manifest). Per DJ-087, refine builds the spec layer — decisions, features, strategies — but does NOT emit approaches; adopt is the spec→code reconciler and owns the synthesis step. For each such parent, adopt dispatches the `synthesizer` subagent with the parent body + applicable decisions, calls `spec_propose_approach` with the returned body and id `app-<parent-id>` (deterministic; idempotent: on re-run the approach already exists so the parent falls through to `synthesize_and_implement`), and the new approach enters the `synthesize_and_implement` chain in the same iteration. The `synthesize_approach` worklist category precedes all others because an approach must exist before it can be implemented or drift-classified.

5. **Drift classification: set diff + hash diff on both sides.** Step 2 of the adopt playbook computes drift per approach across both spec and code dimensions (renumbered from 4):
