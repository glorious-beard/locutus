...
3. **New daemon-side loop-state MCP tools.** Registered in `internal/mcp/`, backed by per-run state held in the daemon (not the SpecStore — loop state is run-scoped ephemeral bookkeeping, not spec graph content). The tools key on `(activity, target)` — the loop's natural identity, which the agent always re-derives from its run context (the playbook it's running + the `Target:` line), so there is no opaque token to lose across context compression (spike-confirmed, resolved-question 5):
    ...
    A server-internal `run_id` is minted per record for logging/correlation only; the agent never handles it. **Division of labor mirrors the harness exactly:** the *iteration count and cap* are server-tracked and deterministic (no agent self-counting — the original guardrail concern); the *convergence judgment* is the scout's LLM verdict, same as the harness reads from the verdict line headless. The tools don't judge convergence; they record it and enforce the cap.

4. **The loop identity is `(activity, target)` (the "session ID notion"), resolved by the spike.** It is re-derivable from the agent's run context, so it survives context compression with nothing to remember. Fresh-run-vs-recovery is distinguished by **record lifecycle, not the key**: a record is GC'd when its loop terminates (converged or capped) or on a TTL, so a genuinely fresh `/locutus-refine` after a prior converged run finds no live record and starts at iteration 1, while a post-compression re-call mid-run finds the live record and resumes. Distinct from the `.locutus/sessions/<…>/<sid>/` directory id (a dispatch artifact, headless-only, never reaches the daemon). The per-project daemon singleton holds independent records per `(activity, target)`, so sequential refine-then-import in one interactive session don't collide. Sole acknowledged edge: two *concurrent* loops on the same `(activity, target)` in one daemon (e.g. two operators) would merge — acceptable for the single-operator interactive scenario this targets, and they'd conflict on the spec graph anyway.
...
5. **The loop is keyed on `(activity, target)` — spike-resolved (chat 2026-05-27).** The deciding criterion was *what the agent is least likely to lose track of mid-run*, because an interactive self-loop can undergo context compression between iterations. The spike (`internal/mcp` + go-sdk v1.6.1) ruled out the two alternatives and confirmed the keying:
    - **(c) Server resolves from a per-invocation MCP session — NOT VIABLE.** The go-sdk's IO/socket transport hardcodes `ioConn.SessionID()` → `""` (server-issued session ids are streamable-HTTP-only), so a handler observes no session id. And the bridge collapses granularity regardless: `locutus mcp` launches once per coding-agent session, so one socket connection = one `ServerSession` for the whole interactive session — sequential `/locutus-refine` and `/import` share it. Wrong granularity even if an id existed.
    - **(b) Agent's ambient runtime session id — UNVERIFIABLE here.** It's a runtime-side fact (does Codex/Gemini expose a stable readable session id to the agent?); the repo's ACP `SessionID` lives on the headless dispatch side and never reaches the MCP daemon, and interactive `/locutus-refine` doesn't go through the ACP runner at all. Not assumable.
    - **(a, chosen + refined) `(activity, target)` natural key.** The agent re-derives both from its run context every call — nothing opaque to remember, maximally compression-robust. The earlier worry that `(activity, target)` "can't tell a fresh run from a recovery" is resolved by **record lifecycle**: a terminated (converged/capped/TTL'd) record isn't recoverable, so a fresh invocation starts clean while a live record resumes. A server-internal `run_id` is kept for logging only.
    This is the most-robust point on the original ranking, reached by combining the rejected-bare-token's recovery property with the natural key. The tool signatures in Decision point 3 are now concrete.
