...
| `spec_loop_begin` | `{activity, target}` → `{iteration, max_iterations}` | Starts (or recovers) the interactive self-loop's iteration record for the named run (DJ-142). Allocates a fresh record at iteration 1 when none is live for `(ServerSession, activity, target)`, or returns the live one's current iteration when it exists — the compression-recovery path: an agent that lost its place after a context compression re-calls this and resumes mid-loop. `max_iterations` is read from the activity registry (DJ-138), so `.borg/agents.yaml` overrides apply. The tier-3 `spec_refinement.interactive.md` playbook calls this once before its first pass. |
