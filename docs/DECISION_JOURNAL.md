...
**Status:** designed; implementation not started (plan at [.claude/plans/dj-136-per-runtime-idiomatic.md](../.claude/plans/dj-136-per-runtime-idiomatic.md)).
...
**Status:** designed; implementation not started (plan at [.claude/plans/dj-137-justify-activity.md](../.claude/plans/dj-137-justify-activity.md)). Assumes [DJ-136](#dj-136-per-runtime-idiomatic-dispatch-layer--playbook-overlays-hook-configs-and-native-goal-loops-refines-dj-135s-cross-runtime-contract-stance-reverses-resolved-question-14-adds-runtimemd-playbook-overlay-convention-to-internalscaffoldplans-introduces-hook-configs-as-a-per-runtime-publisher-output-asymmetric-convergence-goal-driven-on-claude-code-locutus-driven-on-codex--gemini-per-runtime-use-of-each-providers-idiomatic-affordances-replaces-the-lowest-common-denominator-floor) has shipped first (specifically Phase 2's plan-update event surfacing in the runner, which justify will benefit from when the orchestrator emits its iteration plan via `TodoWrite`).
