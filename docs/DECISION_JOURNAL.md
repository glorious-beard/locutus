...
- **Code (add):**
    - [internal/agent/spec_store.go](../internal/agent/spec_store.go) (new) — the unified `SpecStore`. Holds typed slices for decisions, features, strategies, bugs, approaches; per-entry `Origin` and `Working` flags; a `sync.RWMutex`; a write-through Bluge index. Exposes `Put` / `Get` / `List` / `Search` / `MarkWorking` / `ClearWorking` / `Persist` / `Discard` / `LoadFromFS`. ~400-500 lines projected.
...
- **Documentation:**
    ...
    - `docs/agent-conventions.md` — no change required (the conventions are about prompt/schema design; the unified store is transparent at that layer).
