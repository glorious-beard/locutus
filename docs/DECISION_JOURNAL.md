...
2. **Critics have no structured counterproposal.** `CriticIssues` is `Issues []string` — a list of free-form objections. A critic says "dec-X is wrong because Y" but has no schema slot for "and here's the alternative I'd commit to in its place." The elaborator is left guessing what the critic actually wants. The critic, having committed to nothing, can re-raise the same objection (or a refactored version) every iteration with no skin in the game. Compare to `justify_challenger`, which emits a structured `AdversarialConcern{Weakness, Evidence, Counterproposal}` — the challenger has to commit to a specific alternative or retract the concern.
...
2. **Critics emit structured counterproposals.** Replace `Issues []string` with `Issues []CriticIssue{Weakness, Evidence, Counterproposal, RelatedDecisionIDs}` (parallel to `AdversarialConcern`). `Counterproposal` is required — a concrete alternative the critic would commit to (named vendor / config / behavior, not "use something else"). The critic prompt is rewritten: when no concrete counterproposal exists, the critic does not raise the concern — that's the architect's lane, not the critic's. Empty / placeholder counterproposals are rejected at the schema layer.
...
- **Trust the critic to self-regulate.** Keep `Issues []string` but tell the critic in prose "only raise a concern if you'd commit to a specific alternative." Prose alone is not load-bearing — the same kind of failure the DJ-118 invopop schema-tag migration was designed to eliminate. Counterproposal as a required schema field travels into the structured-output mode every adapter uses; prose-only enforcement doesn't. Rejected.
...
- **Code:**
    ...
    - `internal/agent/specgen.go` — `CriticIssues.Issues` shape changes from `[]string` to `[]CriticIssue{Weakness, Evidence, Counterproposal, RelatedDecisionIDs}` with jsonschema tags enforcing non-empty fields and minimum sentence-shape on Counterproposal. `degenerateCriticIssueValidator` rejects empty / placeholder counterproposals (mirrors `degenerateChallengerBrief`).
    - `internal/agent/workflow_spec_generation.go` — `mergeCriticIssues` consumes the new shape; auto-extracts RelatedDecisionIDs via regex as today AND merges with the critic's structured field (critic-provided ids win on conflict). Counterproposal text is carried through to `Concern.Counterproposal` (new `Concern` field).
    ...
    - `internal/scaffold/agents/architect_critic.md`, `cost_critic.md`, `devops_critic.md`, `sre_critic.md` — rewritten to require a Counterproposal field per concern. "Don't raise a concern you wouldn't commit to a specific alternative for" replaces the current "find what doesn't add up" framing for the action-output side; the analysis framing stays as-is.
    - `internal/scaffold/agents/spec_decision_elaborator.md` — Revise mode section rewritten: prior chosen option always becomes the first new alternative entry; the new chosen option is either a flipped prior alternative or a new option with explicit rejection reasoning against the prior alternatives. Walk `docs/agent-conventions.md` end-to-end before drafting per the memory checklist.
    - Tests: `TestMergeDecisionsDemotesPriorChosenOption`, `TestMergeDecisionsRejectsAlternativeShrinking`, `TestCriticIssueRequiresCounterproposal`, `TestRevisionCapCommitsRatherThanErrors`, `TestLockedDecisionsAreExcludedFromRevise`, end-to-end smoke test where the loop ships via cap-as-commit on an intractable disagreement.
...
- **Performance:**
    - Critic LLM calls produce ~30-50% more tokens (the Counterproposal field adds prose). Net session cost roughly +10% on critique phases; balanced against the typical -100% from the loop no longer dying at the cap on contradiction-heavy projects.
...
- (a) the structured Counterproposal field causes critics to under-report — they decline to raise concerns whose counterproposal is genuinely uncertain, hiding real issues. Surfaces as critic Issues arrays smaller than under the old prose-only schema on the same fixture inputs. Mitigation: a "counterproposal: needs investigation" sentinel value the critic emits when it sees a real problem but has no concrete alternative; the workflow surfaces those to the user as advisory-only concerns rather than dispatching revisions. If even with the sentinel the critic systematically swallows concerns, the structured shape isn't worth the bullying mitigation and the critic returns to free-form Issues.
