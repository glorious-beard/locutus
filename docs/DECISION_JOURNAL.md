...
**Status:** superseded by [DJ-135](#dj-135-locutus-pivots-from-in-process-council-to-activity-driven-multi-runtime-execution-via-acp-and-mcp-council-becomes-a-coding-agent-executed-playbook-locutus-retains-the-spec-graph--activity-registry-agentsyaml-drives-per-activity-runtime-selection-with-detection-based-fallback-per-project-singleton-mcp-server-coordinates-shared-state-tools-are-the-universal-readwrite-floor--resources-the-additive-human-attach-surface-supersedes-the-workflowexecutor-based-council-architecture-subsumes-dj-127s-write-tools-design-as-the-foundation-layer) on 2026-05-25 — the WorkflowExecutor + spawner-node design retires entirely; activity dispatch via ACP + per-project MCP singleton replaces the Go-encoded graph-mutation runtime. The `internal/agent/workflow*.go` files this DJ shipped were deleted in DJ-135 Phase 5. Prior status: shipping (Phases 1-7 landed 2026-05-14).
...
**Status:** superseded by [DJ-135](#dj-135-locutus-pivots-from-in-process-council-to-activity-driven-multi-runtime-execution-via-acp-and-mcp-council-becomes-a-coding-agent-executed-playbook-locutus-retains-the-spec-graph--activity-registry-agentsyaml-drives-per-activity-runtime-selection-with-detection-based-fallback-per-project-singleton-mcp-server-coordinates-shared-state-tools-are-the-universal-readwrite-floor--resources-the-additive-human-attach-surface-supersedes-the-workflowexecutor-based-council-architecture-subsumes-dj-127s-write-tools-design-as-the-foundation-layer) on 2026-05-25 — the Go-orchestrated council execution layer retires. The council *shape* this DJ defined (decisions-before-narrative; scout-as-judge convergence; the spec-scout / spec-decision-elaborator / spec-feature-elaborator / spec-strategy-elaborator / spec-approach-elaborator / spec-cohesion-critic agent set; the unified import flow) is preserved verbatim as the activity playbook (`internal/scaffold/plans/spec_refinement.md`) — see DJ-135 resolved-question 1 ("the council shape is right; the execution layer is what's brittle"). What retires is the Go-encoded orchestration; what carries forward is every agent prompt and every workflow step's logical sequence. Prior status: shipping (Phases 1-8 landed 2026-05-16).
...
**Status:** superseded by [DJ-135](#dj-135-locutus-pivots-from-in-process-council-to-activity-driven-multi-runtime-execution-via-acp-and-mcp-council-becomes-a-coding-agent-executed-playbook-locutus-retains-the-spec-graph--activity-registry-agentsyaml-drives-per-activity-runtime-selection-with-detection-based-fallback-per-project-singleton-mcp-server-coordinates-shared-state-tools-are-the-universal-readwrite-floor--resources-the-additive-human-attach-surface-supersedes-the-workflowexecutor-based-council-architecture-subsumes-dj-127s-write-tools-design-as-the-foundation-layer) on 2026-05-25 — the InFlightSpecStore overlay retired in [DJ-134](#dj-134-unified-in-process-spec-store-supersedes-the-swappable-wrapper-stack-eliminates-swappablespecsearch--swappablespeclistmanifest--swappablespecget--fsspecprovider--inflightspecstore-single-mutex-protected-store-backs-both-on-disk-persistence-and-in-flight-council-mutations-prepares-the-read-surface-for-tool-call-driven-spec-mutation); the concern model lives in agent-prompt prose now rather than as Go-side bookkeeping. Cap-as-commit specifically retires per DJ-135 resolved-question 13 (subscription economics remove the cost pressure that motivated capping). Prior status: proposed.
...
**Status:** superseded by [DJ-135](#dj-135-locutus-pivots-from-in-process-council-to-activity-driven-multi-runtime-execution-via-acp-and-mcp-council-becomes-a-coding-agent-executed-playbook-locutus-retains-the-spec-graph--activity-registry-agentsyaml-drives-per-activity-runtime-selection-with-detection-based-fallback-per-project-singleton-mcp-server-coordinates-shared-state-tools-are-the-universal-readwrite-floor--resources-the-additive-human-attach-surface-supersedes-the-workflowexecutor-based-council-architecture-subsumes-dj-127s-write-tools-design-as-the-foundation-layer) on 2026-05-25 — decision re-elaboration for cross-decision contradictions retires as a Go-orchestrated mechanism. The discipline carries forward into the playbook (the orchestrator dispatches `spec_revise_decision` MCP tool calls when iter-N critics surface contradictions; the cascade-revisions playbook step covers the cross-node propagation). The Go-side revise-dispatch machinery in `internal/agent/workflow_spec_generation*.go` retires with the rest of the WorkflowExecutor. Prior status: proposed.
...
**Status:** superseded by [DJ-135](#dj-135-locutus-pivots-from-in-process-council-to-activity-driven-multi-runtime-execution-via-acp-and-mcp-council-becomes-a-coding-agent-executed-playbook-locutus-retains-the-spec-graph--activity-registry-agentsyaml-drives-per-activity-runtime-selection-with-detection-based-fallback-per-project-singleton-mcp-server-coordinates-shared-state-tools-are-the-universal-readwrite-floor--resources-the-additive-human-attach-surface-supersedes-the-workflowexecutor-based-council-architecture-subsumes-dj-127s-write-tools-design-as-the-foundation-layer) on 2026-05-25 — the dimension-driven critic *discipline* (scout surfaces critique_dimensions; the cohesion-critic dispatches per-dimension rather than running 4 fixed lenses) carries forward into the activity playbook verbatim. The Go-side dispatch loop that wired this together retires with the WorkflowExecutor. Prior status: shipping (Phases 1-6 landed 2026-05-20).
...
**Status:** shipped 2026-05-23 — the axis-shaped decision ID convention (`dec-<axis-id>`) is foundational and carries forward through [DJ-134](#dj-134-unified-in-process-spec-store-supersedes-the-swappable-wrapper-stack-eliminates-swappablespecsearch--swappablespeclistmanifest--swappablespecget--fsspecprovider--inflightspecstore-single-mutex-protected-store-backs-both-on-disk-persistence-and-in-flight-council-mutations-prepares-the-read-surface-for-tool-call-driven-spec-mutation) and [DJ-135](#dj-135-locutus-pivots-from-in-process-council-to-activity-driven-multi-runtime-execution-via-acp-and-mcp-council-becomes-a-coding-agent-executed-playbook-locutus-retains-the-spec-graph--activity-registry-agentsyaml-drives-per-activity-runtime-selection-with-detection-based-fallback-per-project-singleton-mcp-server-coordinates-shared-state-tools-are-the-universal-readwrite-floor--resources-the-additive-human-attach-surface-supersedes-the-workflowexecutor-based-council-architecture-subsumes-dj-127s-write-tools-design-as-the-foundation-layer). Every decision in the spec graph uses `dec-<axis>` form (verified empirically on winplan refine `~/projects/winplan/.locutus/sessions/20260526/1918/080000/`); the migration shipped as `internal/migrate/MigrateDecisionIDs` and is idempotent on subsequent `locutus update --offline --reset` runs. The MCP `spec_propose_decision` tool registered in DJ-135 backfills `axes` from the id when the agent omits it (per DJ-135 phase-5 convergence-by-construction discipline). CLAUDE.md's "Sources of Truth" section explicitly names this convention as authoritative.
...
**Reference.** Resolves the architectural awkwardness exposed by the seventh winplan re-run (`/Users/chetan/projects/winplan/.locutus/sessions/20260522/1250/10-877ef1/`) — the same trace whose proximal failure mode was addressed by the three-fix change in commit [`eb51a14`]. Builds on [DJ-124](#dj-124-spec-generation-re-architecture--decisions-before-narrative-scout-as-judge-convergence-unified-import-flow-refines-dj-068-spec-graph-topology-replaces-dj-105-inline-decisions-schema-re-scopes-dj-123-in-flight-search)'s axis-driven workflow (the scout surfaces axes; the elaborator commits decisions; under DJ-133 the two share a key namespace). Builds on [DJ-126](#dj-126-decision-re-elaboration-for-cross-decision-contradictions-extends-dj-124-with-existing-decision-revision-depends-on-dj-125-concern-model)'s replace-by-axis-ID match logic by **collapsing** it: the match becomes a trivial ID lookup since the id IS the axis. Builds on [DJ-128](#dj-128-decisions-as-deliberation-logs-structured-critic-counterproposals-revision-cap-as-commit-refines-dj-126-revise-loop-after-third-winplan-re-run)'s cap-as-commit by simplifying the lock-by-axis-id implementation. Honors the three-fix work landed in commit [`eb51a14`] by completing the structural change those mitigations point toward — the prompt corrections, manifest origin/working tags, and inline recovery are necessary in both worlds, but DJ-133's bounded namespace prevents the failure mode the inline recovery has to recover from. Motivated by the seventh winplan re-run trace and the chat 2026-05-22 design discussion of stable-ID-vs-stable-content separation as the underlying invariant the workflow already operates on implicitly.

## DJ-134: Unified In-Process Spec Store Supersedes the Swappable-Wrapper Stack (Eliminates SwappableSpecSearch / SwappableSpecListManifest / SwappableSpecGet / FSSpecProvider / InFlightSpecStore; Single Mutex-Protected Store Backs Both On-Disk Persistence and In-Flight Council Mutations; Prepares the Read Surface for Tool-Call-Driven Spec Mutation)

**Status:** shipped 2026-05-23 — the unified SpecStore is the foundation layer that [DJ-135](#dj-135-locutus-pivots-from-in-process-council-to-activity-driven-multi-runtime-execution-via-acp-and-mcp-council-becomes-a-coding-agent-executed-playbook-locutus-retains-the-spec-graph--activity-registry-agentsyaml-drives-per-activity-runtime-selection-with-detection-based-fallback-per-project-singleton-mcp-server-coordinates-shared-state-tools-are-the-universal-readwrite-floor--resources-the-additive-human-attach-surface-supersedes-the-workflowexecutor-based-council-architecture-subsumes-dj-127s-write-tools-design-as-the-foundation-layer)'s entire MCP server tool surface runs against. No structural change required for DJ-135 — the SpecStore's per-entry typed origin tag (settled / proposed / working) is exactly what the MCP `spec_get` / `spec_search` / `spec_list_manifest` tools expose. Empirically validated against winplan refine on 2026-05-26 (`~/projects/winplan/.locutus/sessions/20260526/1918/080000/`) where the playbook-executed council mutated the store through MCP tool calls and the resulting graph rendered correctly. Phases 1-4 landed 2026-05-23 (commits `3f336dd`, `6accb0c`, `374b173`, `d0e1107`).
...
- **The in-flight overlay only reads the persisted graph by reference-snapshotting it at council start** ([InFlightSpecStore.SetState](../internal/agent/spec_inflight_swap.go#L127)). Edits the council performs to spec nodes via the future write-tool surface ([DJ-127](#dj-127-spec-mutation-tools-as-mcp-write-surface-acp-driven-decision-elaboration-aligns-council-transport-with-subscription-based-ai-funding-model)) would have nowhere to land — the overlay holds `rawProposal` text + an `ExistingSpec` snapshot, not a mutable store. To support mutation-via-tool-call, the overlay would either need to grow its own write path (duplicating the FS path) or punch through to the underlying FS provider, both of which deepen the wrapper layering rather than collapsing it.
...
- **Tool-call-driven spec mutation is on the near-to-medium-term roadmap** (chat 2026-05-23). [DJ-127](#dj-127-spec-mutation-tools-as-mcp-write-surface-acp-driven-decision-elaboration-aligns-council-transport-with-subscription-based-ai-funding-model) frames spec_create / spec_update / spec_delete tools as the surface coding agents and councils alike will use to mutate the graph. Layering write tools on top of the current read-stack would require: a fourth swappable (`SwappableSpecWrite`), a fourth pass-through method on every wrapper, a fourth provider interface, and answering "does the in-flight overlay accept writes or proxy to FS?" — every dimension the read stack already has, doubled.
...
**Reference.** Resolves the latent wrapper-pass-through bug identified in commit [`52e16fd`](https://github.com/chetan/locutus/commit/52e16fd) (the proximal fix); supersedes [DJ-123 Phase 3](#dj-123-spec-search-as-foundational-rag-tool-bm25-only-bluge-corpus-from-spec-graph-deferred-vector-search-supersedes-dj-117-on-corpus-source)'s SwappableSpecSearch pattern; supersedes [DJ-125 Phase 3](#dj-125-concern-driven-revision-and-cap-as-commit) `InFlightSpecStore` overlay design with a unified store. Carries forward the `Origin` (settled / proposed) and `Working` flag work landed in commit [`eb51a14`] — those tags become per-entry fields on the unified store directly. Anticipates [DJ-127](#dj-127-spec-mutation-tools-as-mcp-write-surface-acp-driven-decision-elaboration-aligns-council-transport-with-subscription-based-ai-funding-model)'s write-tool surface: tool-call-driven mutation lands on the same store the council writes through, with one validation seam rather than threading through a separate write-side swappable. Sequenced after [DJ-133](#dj-133-decisions-identified-by-axis-not-chosen-option-eliminates-idcontent-drift-after-flips-collapses-replace-by-axis-id-logic-bounds-the-councils-id-namespace-for-hallucination-defense) ships and validates so the migration-tool work and the store-collapse work don't overlap; DJ-134's surface area is pure internal types and is best landed in isolation. Motivated by the chat 2026-05-23 design critique that named the swap-and-wrap pattern as the load-bearing failure mode behind the wrapper-pass-through bug, and the impending DJ-127 write-tool surface that would compound the layered design's overhead.

## DJ-135: Locutus Pivots from In-Process Council to Activity-Driven Multi-Runtime Execution via ACP and MCP (Council Becomes a Coding-Agent-Executed Playbook; Locutus Retains the Spec Graph + Activity Registry; agents.yaml Drives Per-Activity Runtime Selection with Detection-Based Fallback; Per-Project Singleton MCP Server Coordinates Shared State; Tools Are the Universal Read/Write Floor + Resources the Additive Human-Attach Surface; Supersedes the WorkflowExecutor-Based Council Architecture; Subsumes [DJ-127](#dj-127-spec-mutation-tools-as-mcp-write-surface-acp-driven-decision-elaboration-aligns-council-transport-with-subscription-based-ai-funding-model)'s Write-Tools Design as the Foundation Layer)
...
**Context.** Twelve DJs in five weeks ([DJ-124](#dj-124-spec-generation-re-architecture--decisions-before-narrative-scout-as-judge-convergence-unified-import-flow-refines-dj-068-spec-graph-topology-replaces-dj-105-inline-decisions-schema-re-scopes-dj-123-in-flight-search), [DJ-125](#dj-125-concern-driven-revision-and-cap-as-commit), [DJ-126](#dj-126-decision-re-elaboration-for-cross-decision-contradictions-extends-dj-124-with-existing-decision-revision-depends-on-dj-125-concern-model), [DJ-128](#dj-128-decisions-as-deliberation-logs-structured-critic-counterproposals-revision-cap-as-commit-refines-dj-126-revise-loop-after-third-winplan-re-run), [DJ-129](#dj-129-dimension-driven-critics-scout-surfaced-critique-surfaces-replace-fixed-4-critic-lens-set-builds-on-dj-128-structured-counterproposal-discipline), [DJ-130](#dj-130-provider-mechanics-encapsulated-in-adapters-trace-recording-follows-the-provider-call-boundary-supersedes-unrecorded-5d15e7b-split-in-dispatcher-pattern), [DJ-131](#dj-131-phase-fanout-collapse-for-acp-driven-council-steps-extends-dj-127-with-n-per-session-coding-agent-orchestration-aligns-subscription-quota-math-with-per-refine-fanout-volume), [DJ-132](#dj-132-candidate-survey-agent-for-decision-elaboration-separates-enumeration-from-judgment-to-compress-initial-option-space-extends-dj-124-decision-elaborator-workflow), [DJ-133](#dj-133-decisions-identified-by-axis-not-chosen-option-eliminates-idcontent-drift-after-flips-collapses-replace-by-axis-id-logic-bounds-the-councils-id-namespace-for-hallucination-defense), [DJ-134](#dj-134-unified-in-process-spec-store-supersedes-the-swappable-wrapper-stack-eliminates-swappablespecsearch--swappablespeclistmanifest--swappablespecget--fsspecprovider--inflightspecstore-single-mutex-protected-store-backs-both-on-disk-persistence-and-in-flight-council-mutations-prepares-the-read-surface-for-tool-call-driven-spec-mutation), DJ-103 fill-summaries refinements) have been increasingly tactical responses to a structurally fragile execution model: each addressed a real failure but the cumulative effect is +30 knobs, +10 failure surfaces, and convergence still doesn't reliably land on the first try. Each individual DJ defended its addition; the *trajectory* didn't. Chat 2026-05-23 surfaced the underlying pattern as patch-treadmill — the same DX pathology Locutus was originally designed (in the GaitLab era) to break the operator out of, now reproduced inside Locutus's own development loop.
\
...
13. **Flat-subscription economics retire several tactical fixes from prior DJs.** The shift from per-call token-cost concerns to amortized subscription cost (via runtime-driven LLM calls inside coding agents rather than Locutus's own direct-SDK adapter) removes pressure for: cap-as-commit ([DJ-128](#dj-128-decisions-as-deliberation-logs-structured-critic-counterproposals-revision-cap-as-commit-refines-dj-126-revise-loop-after-third-winplan-re-run)); the thinking/format split ([DJ-130](#dj-130-provider-mechanics-encapsulated-in-adapters-trace-recording-follows-the-provider-call-boundary-supersedes-unrecorded-5d15e7b-split-in-dispatcher-pattern)); revision-count revision caps; the projection token-minimization rationale; the candidate-survey-as-cheaper-than-richer-elaborator framing ([DJ-132](#dj-132-candidate-survey-agent-for-decision-elaboration-separates-enumeration-from-judgment-to-compress-initial-option-space-extends-dj-124-decision-elaborator-workflow)). Most of these mechanisms either retire (when no longer load-bearing) or stay as discipline (when they encode a real cognitive separation independent of cost).
...
- **Code (retain):**
    - `internal/agent/spec_store.go` and the unified SpecStore from [DJ-134](#dj-134-unified-in-process-spec-store-supersedes-the-swappable-wrapper-stack-eliminates-swappablespecsearch--swappablespeclistmanifest--swappablespecget--fsspecprovider--inflightspecstore-single-mutex-protected-store-backs-both-on-disk-persistence-and-in-flight-council-mutations-prepares-the-read-surface-for-tool-call-driven-spec-mutation) — this is the foundation layer; the entire MCP server tool surface runs against it. No structural change required.
    ...
    - `internal/dispatch/acp/` from [DJ-119](#dj-119-agent-client-protocol-replaces-the-coding-agent-driver-layer) — the wire layer for spawning + driving coding agents from the CLI path.
...
- **Code (retire):**
    ...
    - Cap-as-commit machinery from [DJ-128](#dj-128-decisions-as-deliberation-logs-structured-critic-counterproposals-revision-cap-as-commit-refines-dj-126-revise-loop-after-third-winplan-re-run); revision-count limits; per-axis recurrence detection — these were tactical responses to "burning tokens against a moving target." Under flat-subscription economics and coding-agent-driven execution, they're not needed.
...
**Reference.** Synthesizes the design conversation across chat 2026-05-23 (the patch-treadmill diagnosis; the council-shape-is-right framing; the GaitLab DX motivation re-surfacing) and chat 2026-05-24 (the activity-driven architecture; agents.yaml; the publisher discipline; the singleton MCP server; the tools-and-resources surface; the lifecycle decisions). Builds on [DJ-119](#dj-119-agent-client-protocol-replaces-the-coding-agent-driver-layer) (ACP wire layer is the substrate this pivot rests on — already shipped). Subsumes [DJ-127](#dj-127-spec-mutation-tools-as-mcp-write-surface-acp-driven-decision-elaboration-aligns-council-transport-with-subscription-based-ai-funding-model) (the write-tools-as-MCP-surface design becomes the foundation for the MCP server in this DJ; the standalone framing retires in favor of this larger reframe). Builds on [DJ-134](#dj-134-unified-in-process-spec-store-supersedes-the-swappable-wrapper-stack-eliminates-swappablespecsearch--swappablespeclistmanifest--swappablespecget--fsspecprovider--inflightspecstore-single-mutex-protected-store-backs-both-on-disk-persistence-and-in-flight-council-mutations-prepares-the-read-surface-for-tool-call-driven-spec-mutation) (SpecStore is the foundation layer that the MCP server's tool surface runs against — no structural change required). Supersedes the WorkflowExecutor-based council orchestration that grew through [DJ-122](#dj-122-graph-mutation-workflow-executor-with-spawner-nodes-supersedes-dj-112-on-control-flow-topology), [DJ-124](#dj-124-spec-generation-re-architecture--decisions-before-narrative-scout-as-judge-convergence-unified-import-flow-refines-dj-068-spec-graph-topology-replaces-dj-105-inline-decisions-schema-re-scopes-dj-123-in-flight-search), [DJ-125](#dj-125-concern-driven-revision-and-cap-as-commit), [DJ-126](#dj-126-decision-re-elaboration-for-cross-decision-contradictions-extends-dj-124-with-existing-decision-revision-depends-on-dj-125-concern-model), [DJ-128](#dj-128-decisions-as-deliberation-logs-structured-critic-counterproposals-revision-cap-as-commit-refines-dj-126-revise-loop-after-third-winplan-re-run), [DJ-129](#dj-129-dimension-driven-critics-scout-surfaced-critique-surfaces-replace-fixed-4-critic-lens-set-builds-on-dj-128-structured-counterproposal-discipline), [DJ-130](#dj-130-provider-mechanics-encapsulated-in-adapters-trace-recording-follows-the-provider-call-boundary-supersedes-unrecorded-5d15e7b-split-in-dispatcher-pattern), [DJ-131](#dj-131-phase-fanout-collapse-for-acp-driven-council-steps-extends-dj-127-with-n-per-session-coding-agent-orchestration-aligns-subscription-quota-math-with-per-refine-fanout-volume) — these DJs collectively designed and tuned the council that now becomes a playbook. External validation from Stack Overflow Developer Survey 2025 ([survey.stackoverflow.co/2025/ai](https://survey.stackoverflow.co/2025/ai/)) on multi-tool usage patterns; ACP Registry ([agentclientprotocol.com/get-started/registry](https://agentclientprotocol.com/get-started/registry)) on coding-agent runtime coverage; MCP Specification 2025-11-25 ([modelcontextprotocol.io/specification](https://modelcontextprotocol.io/specification)) for the tool/prompt/resource/notification surfaces. The winplan trace at `~/projects/winplan/.locutus/sessions/20260523/1920/18-f5bf96/` is the proximate failure that surfaced the format-pass-drops-decisions pattern and motivated the deeper conversation; the SUCCESS-without-output outcome of that run is the specific failure mode this DJ's pivot eliminates.

## DJ-136: Per-Runtime Idiomatic Dispatch Layer — Playbook Overlays, Hook Configs, and Native Goal-Loops (Refines [DJ-135](#dj-135-locutus-pivots-from-in-process-council-to-activity-driven-multi-runtime-execution-via-acp-and-mcp-council-becomes-a-coding-agent-executed-playbook-locutus-retains-the-spec-graph--activity-registry-agentsyaml-drives-per-activity-runtime-selection-with-detection-based-fallback-per-project-singleton-mcp-server-coordinates-shared-state-tools-are-the-universal-readwrite-floor--resources-the-additive-human-attach-surface-supersedes-the-workflowexecutor-based-council-architecture-subsumes-dj-127s-write-tools-design-as-the-foundation-layer)'s Cross-Runtime-Contract Stance; Reverses Resolved-Question 14; Adds `.<runtime>.md` Playbook Overlay Convention to `internal/scaffold/plans/`; Introduces Hook Configs as a Per-Runtime Publisher Output; Asymmetric Convergence: `/goal`-Driven on Claude Code, Locutus-Driven on Codex / Gemini; Per-Runtime Use of Each Provider's Idiomatic Affordances Replaces the Lowest-Common-Denominator Floor)
...
**Decision.** Reverse [DJ-135](#dj-135-locutus-pivots-from-in-process-council-to-activity-driven-multi-runtime-execution-via-acp-and-mcp-council-becomes-a-coding-agent-executed-playbook-locutus-retains-the-spec-graph--activity-registry-agentsyaml-drives-per-activity-runtime-selection-with-detection-based-fallback-per-project-singleton-mcp-server-coordinates-shared-state-tools-are-the-universal-readwrite-floor--resources-the-additive-human-attach-surface-supersedes-the-workflowexecutor-based-council-architecture-subsumes-dj-127s-write-tools-design-as-the-foundation-layer) resolved-question 14. The playbook content stays cross-runtime where it's portable; everything around it becomes runtime-aware. Specifically:
...
**Reference.** Synthesizes the design conversation across chat 2026-05-26 covering: (a) the diagnosis that prose-bound iteration discipline is the still-fragile residual after DJ-135 shipped; (b) the per-runtime capability fetch ([Claude Code /goal](https://code.claude.com/docs/en/goal), [Codex hooks](https://developers.openai.com/codex/hooks), [Gemini hooks](https://geminicli.com/docs/hooks/)); (c) the strategic framing that LCD is a losing strategy with runtimes racing to parity. Reverses resolved-question 14 of [DJ-135](#dj-135-locutus-pivots-from-in-process-council-to-activity-driven-multi-runtime-execution-via-acp-and-mcp-council-becomes-a-coding-agent-executed-playbook-locutus-retains-the-spec-graph--activity-registry-agentsyaml-drives-per-activity-runtime-selection-with-detection-based-fallback-per-project-singleton-mcp-server-coordinates-shared-state-tools-are-the-universal-readwrite-floor--resources-the-additive-human-attach-surface-supersedes-the-workflowexecutor-based-council-architecture-subsumes-dj-127s-write-tools-design-as-the-foundation-layer). Builds on [DJ-135](#dj-135-locutus-pivots-from-in-process-council-to-activity-driven-multi-runtime-execution-via-acp-and-mcp-council-becomes-a-coding-agent-executed-playbook-locutus-retains-the-spec-graph--activity-registry-agentsyaml-drives-per-activity-runtime-selection-with-detection-based-fallback-per-project-singleton-mcp-server-coordinates-shared-state-tools-are-the-universal-readwrite-floor--resources-the-additive-human-attach-surface-supersedes-the-workflowexecutor-based-council-architecture-subsumes-dj-127s-write-tools-design-as-the-foundation-layer)'s publisher + activity registry as the foundation this layer extends. Per [[feedback-runtime-idiomatic-no-lcd]] (durable preference: per-runtime idiomatic, no LCD) and [[reference-runtime-hook-asymmetry]] (the fetched-and-validated capability matrix). Plan at [.claude/plans/dj-136-per-runtime-idiomatic.md](../.claude/plans/dj-136-per-runtime-idiomatic.md).

## DJ-137: Restore `locutus justify` as an ACP-Dispatched Activity (Same Shape as `refine`; Reuses the Existing `spec-advocate` / `spec-challenger` / `justify-researcher` / `justify-splitter` / `justify-synthesizer` Agent Prompts Still Living in `internal/scaffold/agents/` After [DJ-135](#dj-135-locutus-pivots-from-in-process-council-to-activity-driven-multi-runtime-execution-via-acp-and-mcp-council-becomes-a-coding-agent-executed-playbook-locutus-retains-the-spec-graph--activity-registry-agentsyaml-drives-per-activity-runtime-selection-with-detection-based-fallback-per-project-singleton-mcp-server-coordinates-shared-state-tools-are-the-universal-readwrite-floor--resources-the-additive-human-attach-surface-supersedes-the-workflowexecutor-based-council-architecture-subsumes-dj-127s-write-tools-design-as-the-foundation-layer) Phase 5; New Playbook + Activity Registry Entry + CLI Verb; Builds on [DJ-136](#dj-136-per-runtime-idiomatic-dispatch-layer--playbook-overlays-hook-configs-and-native-goal-loops-refines-dj-135s-cross-runtime-contract-stance-reverses-resolved-question-14-adds-runtimemd-playbook-overlay-convention-to-internalscaffoldplans-introduces-hook-configs-as-a-per-runtime-publisher-output-asymmetric-convergence-goal-driven-on-claude-code-locutus-driven-on-codex--gemini-per-runtime-use-of-each-providers-idiomatic-affordances-replaces-the-lowest-common-denominator-floor)'s Dispatch Layer Without New Per-Runtime Surface — Justify Is One-Shot, Read-Only, and Doesn't Need `/goal` or Hook Configs)
...
- **Restore the Go council just for justify.** Rejected per [[feedback-no-back-compat-until-self-hosting]] and [DJ-135](#dj-135-locutus-pivots-from-in-process-council-to-activity-driven-multi-runtime-execution-via-acp-and-mcp-council-becomes-a-coding-agent-executed-playbook-locutus-retains-the-spec-graph--activity-registry-agentsyaml-drives-per-activity-runtime-selection-with-detection-based-fallback-per-project-singleton-mcp-server-coordinates-shared-state-tools-are-the-universal-readwrite-floor--resources-the-additive-human-attach-surface-supersedes-the-workflowexecutor-based-council-architecture-subsumes-dj-127s-write-tools-design-as-the-foundation-layer)'s "Locutus makes no direct LLM calls under the new model." Re-introducing a Go-side LLM dispatcher for one verb breaks the architectural invariant for marginal gain.
...
- **Performance:** negligible. One ACP session per `locutus justify` invocation, typically 30-90 seconds of wall-clock (one or two subagent dispatches, possibly one researcher web-search pass). Token cost is on the coding-agent runtime's subscription per [DJ-135](#dj-135-locutus-pivots-from-in-process-council-to-activity-driven-multi-runtime-execution-via-acp-and-mcp-council-becomes-a-coding-agent-executed-playbook-locutus-retains-the-spec-graph--activity-registry-agentsyaml-drives-per-activity-runtime-selection-with-detection-based-fallback-per-project-singleton-mcp-server-coordinates-shared-state-tools-are-the-universal-readwrite-floor--resources-the-additive-human-attach-surface-supersedes-the-workflowexecutor-based-council-architecture-subsumes-dj-127s-write-tools-design-as-the-foundation-layer)'s flat-subscription economics.
...
**Reference.** Synthesizes the design conversation on 2026-05-26 (after merging the DJ-135 worktree into main): user surfaced the demand for `locutus justify` to return; verified the agent prompts survived DJ-135 phase 5 council deletion (commit `0731747` deleted Go-side justify code but the five `*-advocate` / `*-challenger` / `justify-*` markdown prompts under `internal/scaffold/agents/` are still in HEAD); confirmed the publisher already emits them to every runtime; identified that no playbook / registry entry / CLI verb exists yet. Assumes [DJ-136](#dj-136-per-runtime-idiomatic-dispatch-layer--playbook-overlays-hook-configs-and-native-goal-loops-refines-dj-135s-cross-runtime-contract-stance-reverses-resolved-question-14-adds-runtimemd-playbook-overlay-convention-to-internalscaffoldplans-introduces-hook-configs-as-a-per-runtime-publisher-output-asymmetric-convergence-goal-driven-on-claude-code-locutus-driven-on-codex--gemini-per-runtime-use-of-each-providers-idiomatic-affordances-replaces-the-lowest-common-denominator-floor) ships first so plan-update event surfacing renders the orchestrator's iteration plan inline during the activity. Plan at [.claude/plans/dj-137-justify-activity.md](../.claude/plans/dj-137-justify-activity.md).


## DJ-138: Add `--with "<bias>"` Strong-Bias Cascade to `locutus refine` for Targeted Override of Deliberation-Layer Nodes — Subsumes the Retired Pre-[DJ-135](#dj-135-locutus-pivots-from-in-process-council-to-activity-driven-multi-runtime-execution-via-acp-and-mcp-council-becomes-a-coding-agent-executed-playbook-locutus-retains-the-spec-graph--activity-registry-agentsyaml-drives-per-activity-runtime-selection-with-detection-based-fallback-per-project-singleton-mcp-server-coordinates-shared-state-tools-are-the-universal-readwrite-floor--resources-the-additive-human-attach-surface-supersedes-the-workflowexecutor-based-council-architecture-subsumes-dj-127s-write-tools-design-as-the-foundation-layer) `--brief` / `--supersede` Flag Surface as a Single Flag with Two-Directional Cascade; Forward Cascade on Decision Targets, Backward+Forward Cascade on Feature/Strategy Targets; Phase-Aware Conservative Drift Marking via `InvalidatedByEventID` Reference-Graph Closure (Day-1) with Witness-State Precision Deferred to a Follow-Up DJ; Per-Activity Iteration Caps Introduced via `internal/activity/agents-default.yaml` Replacing the Hardcoded `const maxIterations = 20` in `internal/runner/run.go`; New `spec_mark_approach_drifted` MCP Tool for Explicit Drift Writes; New `spec_biased` Root History Event with `caused_by` Linking Cascade Children into a Walkable Tree; Best-Effort Mid-Cascade Failure with Git as the Architectural Rollback Layer — No Transactional Spec Mutations; Approach and Bug Nodes Rejected as `refine` Targets Entirely, Goal Additionally Rejected Under `--with`

**Status:** design (designed 2026-05-26 in chat; plan at [.claude/plans/dj-138-refine-with-bias-cascade.md](../.claude/plans/dj-138-refine-with-bias-cascade.md); implementation not yet started; DJ-136 and DJ-137 are prerequisites and ship first per their respective branches).

**Context.** [DJ-135](#dj-135-locutus-pivots-from-in-process-council-to-activity-driven-multi-runtime-execution-via-acp-and-mcp-council-becomes-a-coding-agent-executed-playbook-locutus-retains-the-spec-graph--activity-registry-agentsyaml-drives-per-activity-runtime-selection-with-detection-based-fallback-per-project-singleton-mcp-server-coordinates-shared-state-tools-are-the-universal-readwrite-floor--resources-the-additive-human-attach-surface-supersedes-the-workflowexecutor-based-council-architecture-subsumes-dj-127s-write-tools-design-as-the-foundation-layer) phase 5 retired the pre-existing `refine --brief / --supersede / --diff / --rollback` flag surface alongside the Go council that drove it (commits `915a784` rewiring the CLI verbs to ACP dispatch, `0731747` deleting the council). DJ-135's Status note flagged the retirement with a conditional revival promise: *"the `refine --brief / --supersede / --diff / --rollback` and `history --narrative / --regenerate-narrative` flags retired alongside; if you need them back, they land as ACP-dispatched activities in a follow-up."* [DJ-137](#dj-137-restore-locutus-justify-as-an-acp-dispatched-activity-same-shape-as-refine-reuses-the-existing-spec-advocate--spec-challenger--justify-researcher--justify-splitter--justify-synthesizer-agent-prompts-still-living-in-internalscaffoldagents-after-dj-135-phase-5-new-playbook--activity-registry-entry--cli-verb-builds-on-dj-136s-dispatch-layer-without-new-per-runtime-surface--justify-is-one-shot-read-only-and-doesnt-need-goal-or-hook-configs) is the first redemption of that promise — it restored `justify` as a read-only ACP-dispatched activity. DJ-138 is the second, and it restores the **write-cascade** surface — but with a narrower, more powerful shape than the four-flag pre-DJ-135 vocabulary.
...
3. **New playbook** at `internal/scaffold/plans/spec_bias.md` — cross-runtime default; no `.<runtime>.md` overlay per [DJ-136](#dj-136-per-runtime-idiomatic-dispatch-layer--playbook-overlays-hook-configs-and-native-goal-loops-refines-dj-135s-cross-runtime-contract-stance-reverses-resolved-question-14-adds-runtimemd-playbook-overlay-convention-to-internalscaffoldplans-introduces-hook-configs-as-a-per-runtime-publisher-output-asymmetric-convergence-goal-driven-on-claude-code-locutus-driven-on-codex--gemini-per-runtime-use-of-each-providers-idiomatic-affordances-replaces-the-lowest-common-denominator-floor)'s convention until a per-runtime UX win surfaces. The playbook directs the orchestrator to:
...
4. **Forward cascade semantics (Decision target).** The playbook reads the bias and classifies intent:
    ...
    - The decision's id stays byte-stable across flips per [DJ-133](#dj-133-decisions-identified-by-axis-not-chosen-option-eliminates-idcontent-drift-after-flips-collapses-replace-by-axis-id-logic-bounds-the-councils-id-namespace-for-hallucination-defense)'s axis-shaped id design — backreferences from features/strategies/approaches don't need rewriting.
...
- **Revive the pre-[DJ-135](#dj-135-locutus-pivots-from-in-process-council-to-activity-driven-multi-runtime-execution-via-acp-and-mcp-council-becomes-a-coding-agent-executed-playbook-locutus-retains-the-spec-graph--activity-registry-agentsyaml-drives-per-activity-runtime-selection-with-detection-based-fallback-per-project-singleton-mcp-server-coordinates-shared-state-tools-are-the-universal-readwrite-floor--resources-the-additive-human-attach-surface-supersedes-the-workflowexecutor-based-council-architecture-subsumes-dj-127s-write-tools-design-as-the-foundation-layer) four-flag surface verbatim** (`--brief`, `--supersede`, `--diff`, `--rollback`). Rejected — too wide with overlap, as the user explicitly flagged on 2026-05-26. `--brief` and `--supersede` collapse cleanly into `--with` with playbook-judged intensity; `--diff` and `--rollback` are subsumed by git (`.borg/spec/` lives in version control). Restoring the four-flag surface would also mean re-implementing the supersede id-cascade machinery from scratch under the post-DJ-133 axis-shaped-id model, which doesn't need it (id stability across flips means no id rewriting).
