...
- **Migration:**
    - Phased; the existing council can stay in code as a fallback ("batch mode") while the new path is built. Crossover happens activity-by-activity (refine-goals first; planning second; etc.). Once a calling-agent path produces equal-or-better output than the council path on a representative project (winplan), the council path retires for that activity.
    - Spec graph on disk is unchanged (DJ-134's SpecStore + `.borg/spec/` persistence carries forward). No data migration required.
    - Agent prompt rename pass is a one-shot mechanical change; old prompts continue to work while in `internal/scaffold/agents/` (under-the-hood id translation) until the rename lands. Old projects re-scaffold via `locutus update --reset`.
