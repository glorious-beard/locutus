...
**Why not update agent prompts in this pass.** Phase 5 (DJ-094, DJ-116 reference) updated 19 council prompts to recommend `spec_search` for topic-scoped lookups. The new `matches` map on `SpecSearchHit` is additive: existing prompts still work; an LLM encountering the new field will reason about it from the field names alone. Adoption-shaped follow-up — surfacing the affordance explicitly in prompts — is in scope for a later pass if usage shows agents underweighting the diagnostic.

**What's new:**

- `SchemaVersion` bumped to 2 ([internal/search/search.go](../internal/search/search.go)).
...
- `Options.Explain bool` and `FieldMatch{Terms, Count, Contribution}` types; `Hit.Matches map[string]FieldMatch` ([internal/search/search.go](../internal/search/search.go)).
- `scanPerFieldContributions` and `singleFieldScoreQuery` ([internal/search/index.go](../internal/search/index.go)) implement the per-field scan; `buildMatchDiagnostics` composes them with the Bluge-supplied Locations data into the public `Matches` map.
- Kind-filter clause: `bluge.NewTermQuery(kind).SetField(fieldKind).SetBoost(0)` in `buildQuery`.
- `cmd/list.go`: `RunList` sets `Explain: true`; `ListHit.Matches` (JSON-tagged) carries the diagnostic; `renderListMarkdown` renders `[matched: alternative ×7, rationale ×1]` inline, sorted by contribution descending.
- `internal/agent/spec_tools.go`: `SpecSearchResult.Hits` type widened from `[]SpecManifestEntry` to `[]SpecSearchHit` (id, title, kind, summary, score, matches). Schema-additive — agents that parsed the old shape see every existing field unchanged plus the new ones.
- `TestBluge_PerFieldScoreEqualsDisjunctionContribution` plus the four Phase 7 tests (`TestSearch_MatchesNilWhenExplainOff`, `TestSearch_MatchesContributionSum`, `TestSearch_MatchesPerFieldSeparation`, `TestSearch_KindFilterNotInMatches`).
...
**Reversal criteria:** revert if (a) `TestBluge_PerFieldScoreEqualsDisjunctionContribution` starts failing on a Bluge upgrade — at which point the design choice is between reimplementing BM25 contributions outside Bluge (heavy) or removing the Contribution field from `FieldMatch` and surfacing only Locations / Counts (lighter, loses the dominant-field-disambiguation use case). (b) The per-field scan cost grows unworkable at corpus scale — mitigations in hand: skip per-field scans for `Explain=false` callers (already done), or cache per-call scan results across hits (already done — the `perFieldContributions` map *is* the cache). (c) Agents systematically ignore the `matches` affordance and the diagnostic becomes inert overhead — at which point the field-split work remains useful for narrower future consumers (operator-facing list rendering, etc.) but the per-field scan plumbing could move behind a separate `Explain` opt-in flag at the MCP-tool surface so agents that don't use it stop paying for it.

**Reference:** depends on DJ-116 (the FTS foundation this builds on — engine choice, field set, fingerprint invalidation, concurrency model), DJ-094 (`spec_list_manifest` / `spec_get` the agent-facing precursor whose ranked counterpart `spec_search` now serves with diagnostics), DJ-114 (authored Summary — the curated text whose match contribution this surface exposes per field). Companion to a future "Phase 7b" pass if agent prompts need explicit guidance on the Matches affordance.
