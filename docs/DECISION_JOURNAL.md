...
|---|---|---|---|
| <a id="dj-152"></a>DJ-152 | Package Locutus as a Claude Code Plugin — MCP Server Descriptor + Verb Skills + Council Agents Assembled From the Canonical Scaffold Sources by a New Publisher Target; Binary Is a PATH Prerequisite (No Bundling, No Bootstrap Downloader); `locutus init` Detects the Installed Plugin and Skips CC Per-Project Publishing; Scope Limited to Packaging — ACP Dispatch, Multi-Runtime Publisher, and Codex/Gemini Treatment Unchanged, Larger Inversion Deferred | settled | [open](decisions/dj-152-claude-code-plugin-packaging.md) |
| <a id="dj-151"></a>DJ-151 | Persona Journey-Walk Enumeration + Ownership-Test Coverage Judgment in `spec-coverage-critic` — Amends DJ-150 After the WinPlan Sufficiency Audit Showed the Critic Converging With the Application Shell Unowned; Citation-Grounded Enumeration Is Structurally Biased Against Tacit Obligations, So the Critic Gains a Second Enumeration Mode Walking Each Derived Persona Through the Full Deliverable Lifecycle (Arrival → Departure); Prose Coverage (DJ-150 RQ3) Reversed to an Ownership Test — Covered Means a Feature Claims the Surface as Its Deliverable With an Acceptance Criterion Exercising It, Never Ambient Mention; Spec Layer Confirmed as the Owning Layer for Deliverable Boilerplate (Explicit and Challengeable Over Implicit); Personas Derived Per-Run, Not Persisted; Scaffold-First Adopt Rejected as Owner and Walkthrough Verification Deferred to a Future DJ | settled | [open](decisions/dj-151-journey-walk-ownership-coverage.md) |
| <a id="dj-150"></a>DJ-150 | `spec-coverage-critic` Surfaces Deliverable-Shape Obligations as Refine-Time Findings — Not as a New Graph Node Kind; Grounded Per-Deliverable Enumeration Mirrors DJ-132's Candidate-Survey Pattern; Features Are the Coverage Target Because Features Are the Recognized PM Artifact and Obligations Are Not; Architect's Revise Pass Extends Existing Feature Scope (Prose Coverage, No `covers` Citation Slice) or Proposes a New Feature to Close the Gap; Coverage Judgment Is Natural-Language Per-Run, Not a Structural Citation Check; Idempotent Across `refine goals` Re-Runs Modulo Grounded-Enumeration Stochasticity and Deliverable-Shape Stability; Triggered by WinPlan's Empty Home-Page Surface After the First End-to-End Adopt Run; Codifies the "Topology vs Process" Bias-Check — New Node Kinds Earn Their Place Only When Each Corresponds to an Independently Recognized PM/Engineering Artifact | settled | [open](decisions/dj-150-spec-coverage-critic.md) |
| <a id="dj-149"></a>DJ-149 | `adopt` Closes the Spec → Code Gap; Runtime-Driven Master Plan via `.locutus/sessions/<sid>/plans/` + Stacked Worktrees (`adopt/<NNN>-<approach-id>`) with Halt-On-First-Failure; Per-Approach State Re-Established at `.borg/state/` Per DJ-068/DJ-096 — Partially Supersedes DJ-148's Misplaced State Fields on `spec.Approach`; Migrates `ReconciliationState.SpecHash` to `SpecHashes map[string]string` for Granular One-Hop Upstream Subgraph Drift Detection; Adds `drift-classifier` Subagent for Semantic-vs-Trivial Code Drift; Honors DJ-068's Test-Asserted-Live-Status Principle; New MCP Tools `state_record_reconciliation` / `state_refresh_artifacts` / `state_mark_status` / `state_delete_record` + Read Tools `state_list_records` / `state_get_record` — All CaptureOnly-Wrapped Per DJ-147; Runtime Decides Parallelism + Branch Ordering Per DJ-144's Trajectory; Closes the Other Half of DJ-135 Phase 5 Checkpoint 3 Leak | settled | [open](decisions/dj-149-adopt-spec-to-code-reconciliation.md) |
| <a id="dj-148"></a>DJ-148 | `assimilate` Closes the Code → Spec Gap; Bidirectional Reconciliation Produces State; Synthesizes Approaches Binding Inferred Specs to Source Files; Code-Is-Truth From This Direction; Preconditions GOALS.md + Goal Layer Populated (Operator Runs `refine goals` First); Reuses Refine's `scout` + Per-Domain Analyzers (`backend-analyzer` / `frontend-analyzer` / `infra-analyzer`) + `gap-analyst` Subagents With Prompts Rewritten for Current Conventions (Axis-Shaped IDs Per DJ-133, No Entity Persistence Per DJ-076); Adds `spec_propose_approach` / `spec_revise_approach` MCP Tools (Shared With DJ-149) + Per-Approach `source_hash` Body Field; Per-Runtime Convergence Drivers Inherited From DJ-144; DJ-147 Dry-Run Inheritance via `captureOnly` Wrapping; Defers Middle-Out Reconciliation, Orphan Handling, Self-Contained Goal-Layer Sync, `code-paths` Cache, and Iterative Hypothesis Cycle to DJ-150+ as Future Work — Closes the DJ-135 Phase 5 Checkpoint 3 Leak | shipped 2026-05-30; state-side fields partially superseded by DJ-149 | [open](decisions/dj-148-assimilate-bidirectional-reconciliation.md) |
| <a id="dj-147"></a>DJ-147 | `--dry-run` on `import` / `refine` / `adopt` / `assimilate` via Per-Session SpecStore Overlay; Capture at the MCP-Tool Boundary by a `captureOnly` Wrapper (Mirroring DJ-143's `requireRuntime`); Activation Through `LOCUTUS_DRY_RUN` + `LOCUTUS_DRY_RUN_FORMAT` Env Vars Forwarded via `_meta`; New Read-Only `spec_dry_run_report` Tool + CLI-Side `tools.jsonl` Render Give a Dual Rendering Path (Agent-Narrated Markdown + Authoritative Headless Structured Output) — Phase B Revival on Post-DJ-135 Footing | settled | [open](decisions/dj-147-dry-run-mutation-capture.md) |
| <a id="dj-144"></a>DJ-144 | Re-Architect the Claude Code Convergence Driver Around Dynamic Workflows — the Workflow Script Becomes the Parallel Orchestrator *and* the Loop for the Four Convergent Activities; Scout Still Owns the Convergence Judgment While the Script Drives Iteration + Enforces the Injected `max_iterations` Cap; Named Subagents Preserved as Workers Behind Phase Barriers; Removes `/goal` + `OuterLoopRunner`-for-Claude-Code + Verdict-Line-as-Harness-Artifact on Claude Code; Codex / Gemini Unchanged — a Fourth Driver for One Runtime, Same Outcome; Vindicates DJ-136; Folds In a Declared Per-Runtime Minimum-Version Registry (Warn-and-Proceed) Generalizing the Workflow v2.1.154 Floor | shipping | [open](decisions/dj-144-cc-workflow-convergence.md) |
