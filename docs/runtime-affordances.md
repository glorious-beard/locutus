...
`locutus adopt` reads the spec graph and state store, identifies approaches needing work (missing for feat/strat in scope / unbound / spec-drifted / code-drifted / orphan-parent), and dispatches the coding-agent runtime to implement them in stacked worktrees. When a Feature or Strategy in scope has no Approach attached, adopt synthesizes one via the `synthesizer` subagent before implementation (per [DJ-087](decisions/dj-087-approaches-are-synthesized-adopt.md) — refine builds the deliberation layer, adopt owns approach synthesis). Each implementation phase: `git worktree add ../<project>-adopt-<NNN>-<approach-id>` off the previous phase's branch (or operator base if phase 1); runtime implements using its native skills; runs the project's test suite (per DJ-068's test-asserted-live principle); calls `state_record_reconciliation` with the diff + test outcome.
